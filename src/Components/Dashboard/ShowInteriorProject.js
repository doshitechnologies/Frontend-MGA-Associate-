...
const ShowInteriorProject = () => {
  ...
  const fetchProjectData = async () => {
    ...
    try {
      const response = await fetch(
        `https://projectassoicate-mt1x.onrender.com/api/interior/interior/${projectId}`
      );
      if (!response.ok) {
  ...
  const handleUpdate = async () => {
    try {
      const response = await fetch(
        `https://projectassoicate-mt1x.onrender.com/api/interior/update/interiors/${editingProject._id}`,
        {
          method: "PUT",
