...
import { FaSearch, FaCouch, FaTrashAlt, FaEye } from "react-icons/fa";
...
const ProjectCard = ({ project, onShowMore, onDelete, isDeleting }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-br from-indigo-800 to-indigo-600 px-5 pt-5 pb-4">
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center flex-shrink-0">
            <FaCouch className="text-white" size={17} />
          </div>
          <TypeBadge type={project.projectType} />
        </div>
        {project.title && (
          <p className="text-indigo-300 text-xs font-semibold uppercase tracking-widest mb-1 truncate">
            {project.title}
          </p>
        )}
        ...
        <p className="text-indigo-300 text-xs mt-1 truncate">
      ...
      <div className="px-5 py-4 flex-1 space-y-0.5">
        ...
        <button
          ...
          className="mt-1 text-xs text-indigo-500 hover:text-indigo-600 font-medium transition-colors"
        >
      ...
      <div className="px-5 pb-5 flex gap-2">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white text-sm font-semibold transition-colors shadow-sm shadow-indigo-200"
        >
    ...
    async (projectId) => {
      ...
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/interior/interiors/${projectId}`,
          {
            method: "DELETE",
            headers: {
        ...
        setProjectData((prevData) => {
          const newData = prevData.filter((p) => p._id !== projectId);
          return newData;
        });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="relative max-w-xs w-full">
          ...
          <input
            type="text"
            placeholder="Search by client name, title, or address…"
            value={searchTerm}
            onChange={(e) => {
              setSearchTerm(e.target.value);
            }}
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent placeholder:text-slate-300"
          />
        </div>
      </div>
      ...
      {current.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 gap-3 text-slate-400">
          <FaCouch size={36} className="opacity-30" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
          {current.map((project) => (
            <ProjectCard
              key={project._id}
              project={project}
              onShowMore={(id) => navigate(`/admin/show/${id}`)}
